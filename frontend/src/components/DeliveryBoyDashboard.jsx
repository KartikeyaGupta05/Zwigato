...
function DeliveryBoyDashboard() {
  ...
  const getAssignments = async () => {
    try {
      const result = await axios.get(
        `${import.meta.env.VITE_BASE_URL}/api/order/get-assignments`,
        {
  ...
  const getCurrentOrder = async () => {
    try {
      const result = await axios.get(
        `${import.meta.env.VITE_BASE_URL}/api/order/get-current-order`,
        { withCredentials: true }
      );
  ...
  const sendOtp = async () => {
    ...
    try {
      const result = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/order/send-delivery-otp`,
        {
  ...
  const verifyOtp = async () => {
    ...
    try {
      const result = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/order/verify-delivery-otp`,
        {
          orderId: currentOrder._id,
          shopOrderId: currentOrder.shopOrder._id,
          otp,
  ...
  const handleTodayDeliveries = async () => {
    try {
      const result = await axios.get(
        `${import.meta.env.VITE_BASE_URL}/api/order/get-today-deliveries`,
        { withCredentials: true }
      );
      console.log(result.data);
  ...
  return (
    <div className="w-screen min-h-screen flex flex-col gap-5 items-center bg-[#fff9f6] overflow-y-auto">
      <DeliveryBoyNavbar />
      <div className="w-full max-w-[800px] mt-25 flex flex-col gap-5 items-center">
        <div className="bg-white rounded-2xl shadow-md p-5 flex flex-col justify-start items-center w-[90%] border border-orange-100 text-center gap-2">
          <h1 className="text-xl capitalize font-bold text-[#ff4d2d]">
